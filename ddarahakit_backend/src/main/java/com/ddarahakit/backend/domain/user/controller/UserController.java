package com.ddarahakit.backend.domain.user.controller;

import com.ddarahakit.backend.common.model.BaseResponse;
import com.ddarahakit.backend.config.security.AuthUserDetails;
import com.ddarahakit.backend.config.security.JwtProvider;
import com.ddarahakit.backend.config.security.token.TokenService;
import com.ddarahakit.backend.config.security.token.TokenService.TokenPair;
...
import com.ddarahakit.backend.domain.orders.OrdersService;
...
public class UserController {
    ...
    private final com.ddarahakit.backend.domain.study.StudyService studyService;
    ...
    @GetMapping("/myreview")
    public ResponseEntity<BaseResponse<List<ReviewDto.MyReviewRes>>> getMyReviewList(
            @AuthenticationPrincipal AuthUserDetails authUserDetails
    ) {
        List<ReviewDto.MyReviewRes> response = userService.getMyReviewList(authUserDetails);
        return ResponseEntity.ok(BaseResponse.success(response));
    }

    @Operation(
            summary = "작성한 질문 목록 조회",
            description = "현재 로그인한 사용자가 작성한 질문 목록을 조회한다.")
    @GetMapping("/myquestion")
    public ResponseEntity<BaseResponse<List<CommunityDto.PostSummaryResponse>>> getMyQuestionList(
            @AuthenticationPrincipal AuthUserDetails authUserDetails
    ) {
        List<CommunityDto.PostSummaryResponse> response = userService.getMyQuestionList(authUserDetails);
        return ResponseEntity.ok(BaseResponse.success(response));
    }

    @Operation(
            summary = "작성한 게시글 목록 조회",
            description = "현재 로그인한 사용자가 작성한 게시글 목록을 조회한다.")
    @GetMapping("/mypost")
    public ResponseEntity<BaseResponse<List<CommunityDto.PostSummaryResponse>>> getMyPostList(
            @AuthenticationPrincipal AuthUserDetails authUserDetails
    ) {
        List<CommunityDto.PostSummaryResponse> response = userService.getMyPostList(authUserDetails);
        return ResponseEntity.ok(BaseResponse.success(response));
    }

    @Operation(
            summary = "수강 중 강의 조회",
            description = "현재 로그인한 사용자가 구매한 코스 목록을 조회한다.")
    @GetMapping("/ordered")
    public ResponseEntity<BaseResponse<List<CourseDto.CourseRes>>> getOrderedCourseList(
            @AuthenticationPrincipal AuthUserDetails authUserDetails
    ) {
        List<CourseDto.CourseRes> response = userService.getOrderedCourseList(authUserDetails);
        return ResponseEntity.ok(BaseResponse.success(response));
    }

    @Operation(
            summary = "주간 학습활동 조회",
            description = "최근 7일 일별 수강완료 수/연속학습일/주간목표 달성률. 출석=하루 1강 이상 완료 기준. " +
                    "completed_at 신규 컬럼 기반이라 과거(시각 NULL) 완료는 제외됨.")
    @GetMapping("/study/weekly")
    public ResponseEntity<BaseResponse<com.ddarahakit.backend.domain.study.StudyDto.WeeklyRes>> getWeeklyStudy(
            @AuthenticationPrincipal AuthUserDetails authUserDetails
    ) {
        return ResponseEntity.ok(BaseResponse.success(studyService.weekly(authUserDetails)));
    }

    @Operation(
            summary = "결제 내역 조회",
            description = "현재 로그인한 사용자의 결제 완료 내역을 페이징으로 조회한다.")
    @GetMapping("/payments")
